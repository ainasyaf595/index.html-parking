...
<body>
  ...
  <script>
    const firebaseConfig = {
      ...
      databaseURL: "https://smart-parking-aee-default-rtdb.asia-southeast1.firebasedatabase.app",
      projectId: "smart-parking-aee",
      storageBucket: "smart-parking-aee.appspot.com",
    ...
    // Listener untuk setiap lot
    database.ref("parking/lot1").on("value", snap => updateLot("lot1", snap.val()));
    database.ref("parking/lot2").on("value", snap => updateLot("lot2", snap.val()));
    database.ref("parking/lot3").on("value", snap => updateLot("lot3", snap.val()));
    database.ref("parking/lot4").on("value", snap => updateLot("lot4", snap.val()));
    database.ref("parking/lot5").on("value", snap => updateLot("lot5", snap.val()));
  </script>
